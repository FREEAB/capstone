...
dropdowns.forEach((dropdown) => {
    ...
    // Converts dropdown IDs from 'X_dropdown_Y' to 'dropdown_XYYYY-MM-DD
    let dropdownID = dropdown.id.split("")[0];
    let dropdownDay = dropdown.id.split("")[2];
    let dropdownDate = dayMappings[dropdownDay];
    dropdown.id = `dropdown${dropdownID}_${dropdownDate}`;

    // Make event listener for changes to change colors and save changes to db
    dropdown.addEventListener('change', (event) => {
        const dropdownValue = event.target.value;
        //if drop down value equal zero get userid
        if (dropdownValue == 0) {
            const dropdownID = event.target.id.split("")[1];

            // Simple POST request to /api/email with user_id in body
            fetch('/api/email', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
